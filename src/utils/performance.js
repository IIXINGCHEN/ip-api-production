...
export class PerformanceTimer {
  ...
  recordMetric(duration) {
    ...
    const existing = metrics.requests.get(key) || { count: 0, total: 0, min: Infinity, max: 0 }
...
export function trackCachePerformance(operation, key, hit = false) {
  if (!isFeatureEnabled('monitoring')) {return}

  const cacheKey = `cache:${operation}`
  const existing = metrics.cache.get(cacheKey) || { hits: 0, misses: 0, total: 0 }
...
export function trackError(error, context = {}) {
  ...
  const existing = metrics.errors.get(errorKey) || { count: 0, lastSeen: 0, contexts: [] }
...
export function getHealthStatus() {
  ...
  // Check error rates
  const totalErrors = Object.values(metrics.errors).reduce((sum, e) => sum + e.count, 0)
  const totalRequests = Object.values(metrics.responses).reduce((sum, r) => sum + r.total, 0)
  ...
  const avgResponseTime = Object.values(metrics.requests).reduce((sum, r) => sum + r.avgDuration, 0) / Object.keys(metrics.requests).length
