...
export class ThreatService {
    ...
    try {
      ...
      checks.forEach((result, index) => {
        if (result.status === 'fulfilled' && result.value) {
          const checkName = ['VPN', 'Proxy', 'Tor', 'Bot', 'Reputation', 'Malicious'][index]
          ...
          if (data.detected) {
            ...
            const weight = this.rules.riskWeights[`${checkName.toLowerCase()}Pattern`] || data.riskScore || 10
  ...
  async checkBot(_ip, request) {
    try {
      ...
      if (!request.header('accept-language')) { riskScore += 5 }
      if (!request.header('accept-encoding')) { riskScore += 5 }
      if (!userAgent) { riskScore += 20 }

      if (riskScore > 10) { detected = true }
  ...
  isKnownVPNRange(ip) {
    ...
    if (ip.includes(':')) {
      return this.rules.knownVPNRanges.ipv6.some(range => ip.startsWith(range))
    } else {
      return this.rules.knownVPNRanges.ipv4.some(range => ip.startsWith(range))
    }
  }
  ...
  isLegitimateISP(ip) {
    ...
    if (ip.includes(':')) {
      return this.rules.legitimateISPs.ipv6.some(range => ip.startsWith(range))
    } else {
      return this.rules.legitimateISPs.ipv4.some(range => ip.startsWith(range))
    }
  }
  ...
  isDatacenterIP(ip) {
    ...
    return this.rules.datacenterRanges.ipv4.some(range => ip.startsWith(range))
  }
  ...
  isKnownProxyRange(ip) {
    ...
    const proxyRanges = [
      ...
      '185.220.100.', '185.220.101.', // Tor exits used as proxies
      '198.98.50.', '198.98.51.', // Known proxy services
      '46.166.160.', '46.166.161.', // Specific proxy ranges
      '192.42.116.', // Known proxy service

      // Additional known proxy ranges
      '5.79.', '5.135.', // OVH proxy services
      '51.15.', '51.158.', // Scaleway proxy services
  ...
  isLegitimateService(ip) {
    ...
    return this.rules.legitimateServices.ipv4.some(range => ip.startsWith(range))
  }
  ...
  analyzeProxyHeaders(request) {
    ...
    const cfHeaderCount = indicators.filter(i => i.includes('cloudflare')).length
