...
const geoQuerySchema = z.object({
  ...
  include_threat: z.string().optional().transform(val => val === 'true')
})
...
app.get('/geo/:ip', zValidator('param', ipSchema), zValidator('query', geoQuerySchema), async (c) => {
  try {
...
app.post('/geo/batch', zValidator('json', z.object({
  ...
  try {
    ...
    const results = await Promise.allSettled(
      ips.map(async (ip) => {
        ...
        const requestedFields = options.fields ? options.fields.split(',') : null
